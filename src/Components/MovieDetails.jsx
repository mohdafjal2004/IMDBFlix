...
const MovieDetails = () => {
  ...
  const fetchDetails = () => {
    axios
      .get(`/${state.menuType}/${id}?api_key=${key}&language=en-US`)
      .then((response) => setApi(response.data));
  };

  const fetchCast = () => {
    axios
      .get(`/${state.menuType}/${id}/credits?api_key=${key}&language=en-US`)
      .then((response) => setCast(response.data));
  };

  const similar = () => { 
    axios
      .get(`/${state.menuType}/${id}/similar?api_key=${key}&language=en-US`)
  ...
  return (
    <div className="parent">
        ...
        >
          ...
          <div className="text-white ml-3 sm:ml-7 xl:pb-[150px] 2xl:pb-[170px]  sm:text-left sm:w-1/2 pt-8 space-y-2 h-auto relative ">
            ...
            {state.menuType === "movie" ? (
              ...
              <h1>Airs: Monday to Friday, 9:00 PM</h1>
            )}
      ...
      {/* All Casts */}
      {state && state.menuType && state.menuType !== "tv" && (
